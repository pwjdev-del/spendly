...
import { UploadCloud, CheckCircle, AlertTriangle, FileText, Loader2, Trash2, Repeat, Settings } from "lucide-react"
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-foreground">
                        {showSubscriptionManager ? "Subscription Manager" : "Reconciliation Command Center"}
                    </h1>
                    <p className="text-muted-foreground mt-1">
                        {showSubscriptionManager ? "Track recurring expenses and optimize costs." : "AI-powered ledger verification and anomaly detection."}
                    </p>
                </div>
                <div className="flex items-center gap-4">

                    {isSubscriptionFeatureEnabled && (
                        <div className="flex items-center gap-2 bg-secondary/50 p-1.5 rounded-full border border-border">
                            <span className={cn("text-xs font-medium px-3 py-1.5 rounded-full transition-colors", !showSubscriptionManager ? "bg-background shadow-sm text-foreground" : "text-muted-foreground cursor-pointer")} onClick={() => setShowSubscriptionManager(false)}>Transactions</span>
                            <span className={cn("text-xs font-medium px-3 py-1.5 rounded-full transition-colors flex items-center gap-1.5", showSubscriptionManager ? "bg-background shadow-sm text-primary" : "text-muted-foreground cursor-pointer")} onClick={() => setShowSubscriptionManager(true)}>
                                Subscriptions
                            </span>
                        </div>
                    )}

                    {!showSubscriptionManager && report && (
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="icon" className="rounded-full bg-secondary/30 border-white/10">
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuCheckboxItem
                                checked={isSubscriptionFeatureEnabled}
                                onCheckedChange={(checked) => {
                                    setIsSubscriptionFeatureEnabled(checked)
                                    if (!checked) setShowSubscriptionManager(false)
                                }}
                            >
